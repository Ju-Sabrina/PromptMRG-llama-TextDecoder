...
class GPULowUtilization(gpustats.GPUOperation):
    ...
    display_name = "GPU Low Utilization"
    usage = f"""{{SCRIPT}}[:options...] -- GPU Low Utilization
    ...
    Options:
        ...
        chunks=<number> - Number of equal-duration chunks in which the GPU time
            utilization is calculated for each device in a process. If the
            profile duration cannot be evenly divided by <number>, the duration
            of every chunk is rounded up to the nearest integer nanosecond,
            such that all chunks still have the same duration.
...
"""
    ...
    def setup(self):
        ...
        if self._chunks and not 1 <= self._chunks <= 1000:
            parser.error("argument --chunks: value must be between 1 and 1000")
